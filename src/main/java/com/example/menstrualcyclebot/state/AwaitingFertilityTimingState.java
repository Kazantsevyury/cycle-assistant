...
@Slf4j
public class AwaitingFertilityTimingState  implements UserStateHandler {
    private final long chatId;
    private final Integer messageId;

    public AwaitingFertilityTimingState(long chatId, Integer messageId) {
        this.chatId = chatId;
    ...
    public void handleState(Bot bot, Update update, Cycle cycle) {
        ...
        try {
            ...
            bot.getUserService().updateFertilityRecommendationTiming(chatId, time.toString());
            ...
            if (messageId != null && messageId > 0) {
                try {
                    EditMessageReplyMarkup replyMarkup =
                            bot.getNotificationService().createFertilityWindowMenu(chatId, messageId);
