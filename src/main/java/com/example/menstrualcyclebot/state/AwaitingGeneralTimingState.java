...
@Slf4j
public class AwaitingGeneralTimingState implements UserStateHandler {
    private final long chatId;
    private final Integer messageId;

    // ✅ Правильный конструктор, принимающий messageId
    public AwaitingGeneralTimingState(long chatId, Integer messageId) {
        this.chatId = chatId;
    ...
    public void handleState(Bot bot, Update update, Cycle cycle) {
        ...
        try {
            ...
            bot.getUserService().updateGeneralRecommendationTiming(chatId, time.toString());
            ...
            if (messageId != null && messageId > 0) {
                try {
                    EditMessageReplyMarkup replyMarkup =
                            bot.getNotificationService().createGeneralNotificationSettingsMenu(chatId, messageId);
